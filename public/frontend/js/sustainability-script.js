$(document).ready(function() {
    ...
    // Navbar Scroll Effect - Always scrolled on sustainability page
    $('#mainNavbar').addClass('scrolled');
    
    $(window).on('scroll', function() {
        // Keep scrolled class always on sustainability page
        $('#mainNavbar').addClass('scrolled');
    });
    ...
    $(window).on('scroll', function() {
        ...
        var heroSection = $('.sustainability-hero-section');
        var heroHeight = heroSection.outerHeight();
        
        if (scrolled < heroHeight) {
            var parallaxSpeed = scrolled * 0.5;
            $('.hero-zoom-image').css('transform', 'translateY(' + parallaxSpeed + 'px) scale(' + (1 + scrolled * 0.0003) + ')');
        }
    });

$(document).ready(function() {
    ...
    // Navbar Scroll Effect - Always scrolled on ethical sourcing page
    $('#mainNavbar').addClass('scrolled');
    
    $(window).on('scroll', function() {
        // Keep scrolled class always on ethical sourcing page
        $('#mainNavbar').addClass('scrolled');
    });
    ...
    $(window).on('scroll', function() {
        ...
        var heroSection = $('.ethical-hero-section');
        var heroHeight = heroSection.outerHeight();
        
        if (scrolled < heroHeight) {
            var parallaxSpeed = scrolled * 0.5;
            $('.ethical-hero-section .hero-zoom-image').css('transform', 'translateY(' + parallaxSpeed + 'px) scale(' + (1 + scrolled * 0.0003) + ')');
        }
    });
